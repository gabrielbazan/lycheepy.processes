...
class ProcessesGateway(object):
    ...
    def get_instance(self, identifier, local_path):
        ...
        module_name, full_path = self.get_local_process_file(local_process_dir)
        sys.path.insert(0, full_path)
        import_module(module_name)

        instance = None
        for name, _class in inspect.getmembers(sys.modules[module_name], inspect.isclass):
            if _class.__name__ == identifier:
                instance = _class()
    ...
    def get_local_process_file(local_process_dir):
        process_dir_files = [f for f in listdir(local_process_dir) if isfile(join(local_process_dir, f))]
        return process_dir_files[0], join(local_process_dir, process_dir_files[0])
